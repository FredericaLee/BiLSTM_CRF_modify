...
def data_pre1():
    files = open('F:/intern/test/jie',encoding='UTF-8') #训练文件目录
    train = []
    train_data=[]
    sen_x =[ ]          #句子x
    sen_y =[ ]          #句子y
...
tag_to_ix = {"B-LOC": 0, "I-LOC": 1, "B-ORG": 2, "I-ORG": 3,"B-PER": 4,"I-PER": 5,"O": 6,START_TAG: 7, STOP_TAG: 8}
...
for epoch in range(
    ...
    for sentence, tags in training_data:
        ...
        loss.backward()
        optimizer.step()
    print("epoch: "+str(ep), end = " ")
    ep+=1
    print(time.clock()-etime)
...
def data_pre2():
    data_test = []
    test_t2 = []
    file_test = open('F:/intern/train/trains','r',encoding='UTF-8')
...
with torch.no_grad():
    file_re = open('F:/intern/test/LSTM_re', 'w+', encoding='UTF-8')
    for test in test_data:
        pre = prepare_sequence(test,word_to_ix)
        re = model(pre)
        result = str(re)
        file_re.write(result + "\n")
        print(result)
